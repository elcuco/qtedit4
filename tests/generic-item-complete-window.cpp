...
SuggestionModel::SuggestionModel(QObject *parent) : QAbstractTableModel(parent) {
    // m_suggestions = suggestions;
}

int SuggestionModel::rowCount(const QModelIndex &parent) const { return m_suggestions.count(); }

int SuggestionModel::columnCount(const QModelIndex &parent) const { return 2; }
...
QVariant SuggestionModel::data(const QModelIndex &index, int role) const {
    ...
    switch (index.column()) {
    case 0:
        return m_suggestions.at(index.row()).value;
    case 1:
        return m_suggestions.at(index.row()).fileName;
    default:
        return QVariant();
        break;
    }
...
void GenericItemWindow::initGUI() {
    ...
    connect(m_files_filter, &QLineEdit::textEdited,
            [this]() { filesFilterModel->setFilterFixedString(m_files_filter->text()); });
}
...
void GenericItemWindow::initFolders() { directoryModel->addDirectory(QDir::currentPath()); }

int LevenshteinDistance(const QString &s, const QString &t);
int DiegoDistance(QString s, QString t);

bool suggestionLessThan(const Suggestion &s1, const Suggestion &s2) { return s1.value < s2.value; }
...
void GenericItemWindow::fillSuggestions(QString requestedSuggestion,
    ...
    for (int i = 0; i < max; i++) {
        ...
        if (d < 10 &&
            (itemName.endsWith(".c") || itemName.endsWith(".cpp") || itemName.endsWith(".cxx"))) {
            ss.value = ss.value * 1.2 + 20;
        }
        if (d < 10 && (itemName.endsWith(".h") || itemName.endsWith(".hpp"))) {
            ss.value = ss.value * 1.2 - 10;
        }
        if (itemName.endsWith(".o") || itemName.endsWith(".so") || itemName.endsWith(".obj") ||
            itemName.endsWith(".dll") || itemName.endsWith(".lib") || itemName.endsWith(".a") ||
...
int LevenshteinDistanceIterative(const QString &ss, const QString &tt) {
    ...
    for (unsigned int i = 0; i < len1; i++) {
        ...
        for (unsigned int j = 0; j < len2; j++) {
            col[j + 1] =
                min(min(1 + col[j], 1 + prevCol[1 + j]), prevCol[j] + (s[i] == t[j] ? 0 : 1));
        }
        col.swap(prevCol);
    }
    return prevCol[len2];
...
int LevenshteinDistanceRecursive(const QString &s, const QString &t) {
    ...
    t1.chop(1);
    t1.remove(0, 1);
    return minimum(LevenshteinDistance(s1, t) + 1, LevenshteinDistance(s, t1) + 1,
